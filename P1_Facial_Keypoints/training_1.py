...
class DataGenerator(keras.utils.Sequence):
    ...
    def __get_data(self, batch):
        ...
        for i, id in enumerate(batch):
            ...
            randAngle = random.randint(-7, 7)
            zoomXRange = random.randint(-7, 7)
            zoomYRange = random.randint(-7, 7)
...
#             print(saved)
            ...
            result_img = cv2.resize(result_img, (160,160), interpolation = cv2.INTER_AREA)
            
            final_label = final_label.reshape(-1, 2)
            final_label[:, 0] = (final_label[:, 0]/saved[1])*160
            final_label[:, 1] = (final_label[:, 1]/saved[0])*160
...
model.load_weights("training_7_22.ckpt")

model.fit(gen,steps_per_epoch=steps, epochs=2500, callbacks = [tboard_callback])

model.save_weights("training_7_23.ckpt")
